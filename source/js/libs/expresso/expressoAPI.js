...
var ExpressoAPI = new function() {
	...
	this.triggerCacheRequest = function(currentCache,currentRequest) {
		...
		//console.log("NEWCACHE - LENGTH: " +newCache.length);
	...
	this.addToCache = function(request,response) {
		...
		var saveCache = function(request,response,currentCache) {
			...
			//console.log(JSON.stringify(currentCache));
			//console.log("CURRENTCACHE - LENGTH: " + currentCache.length);
			...
			//console.log("CACHE - SAVE - LENGTH: " + that._cache.length);

			that.setLocalStorageValue(that.getCacheID(),that._cache);
		};

		var foundCallBack = function(cacheResponse) {
			//console.log("foundCallBack - FROM CACHE");
	...
	this.getPhonegapDatabaseValue = function(tx) {
		//console.log('getPhonegapDatabaseValue()');
		...
		var querySuccess = function(tx,results) {
			if (results) {
				...
				//console.log("QUERY-LENGTH:" + results.rows.length);
			    for (var i=0; i<len; i++){
	...
	this.getPhonegapCacheValue = function(tx) {
		//console.log('getPhonegapCacheValue()');
	...
	this.setLocalStorageValue = function(c_name,value) {
		//console.log('setLocalStorageValue(' + c_name + ',' + value + ')');
		// if (_phoneGap && _android) {
			
		// 	if (c_name == 'expresso') {
		// 		_tempData = value;
		// 		this._db.transaction(this.updatePhonegapDatabase, this.errorCB, this.successCB);
		// 	}
		// 	if (c_name == 'expressoCache') {
		// 		_tempCacheData = value;
		// 		this._db.transaction(this.updatePhonegapCache, this.errorCB, this.successCB);
		// 	}
		...
		// } else {
			if (window.localStorage != undefined) {
				try {
	            	...
	            	//console.log("Não foi possível salvar no CACHE - PROVAVELMENTE O ESPAÇO ACABOU - LIMPANDO TODO O CACHE PARA GANHAR MAIS ESPAÇO");
	                return false;
	            }
	...
	this.getLocalStorageValue =  function(name,successCallBack,failCallBack) {
		//if (name == 'expresso') {
			//console.log('getLocalStorageValue(' + name + ',' + successCallBack + ',' + failCallBack + ')');
			//console.log('getLocalStorageValue(' + name + ',successCallBack,failCallBack)');
		...
		// 		this._db.transaction(this.getPhonegapDatabaseValue, this.errorCB,this.successCB);
		// 	}

		// 	if (name == 'expressoCache') {
		// 		_tempCallbackCache = successCallBack;
		// 		this._db.transaction(this.getPhonegapCacheValue, this.errorCB,this.successCB);
		// 	}
	...
	this.errorCB = function(err) {
        //alert("Erro processando SQL: "+err.code);
    };

    this.successCB = function(data) {
    	//console.log("successCB:");
    	//console.log(this._tempResult);
	...
	this.createPhoneGapDatabase = function() {
		// if (_phoneGap && _android) {
		// 	this._db = window.openDatabase("expresso", "1.0", "Expresso Mobile", 200000);
		//     this._db.transaction(this.populatePhoneGapDatabase, this.errorCB, this.successCB);
		//     this._db.transaction(this.getPhonegapDatabaseValue, this.errorCB, this.successCB);
	 //    }
	};
			...
			var successCallBackCache = function(foundCache) {
				...
				if ( (foundCache != false) && ((foundCache.response.result != undefined) && (foundCache.response != undefined)) ) {
					if ((foundCache.response.result) && (foundCache.response)) {
						...
						// console.log(JSON.stringify(foundCache));
						// console.log(JSON.stringify(that._data[that.id()]));
						// console.log(that._data[that.id()]);
						...
						jQuery.ajax(conf).done(function(response) {
							...
							if (response && response.result) {
								...
								if (_debug) {
									...
									console.log(JSON.stringify(response));
									// alert("FAIL 2: " + JSON.stringify(_data[this.id].send.params));
									// alert(JSON.stringify(response));
								}
									...
									var forceCache = function(forcedCacheResponse) {
										if (forcedCacheResponse) {
											if ((forcedCacheResponse.response.result) && (forcedCacheResponse.response)) {
												if (_tempDataset[_tempId].done) _tempDataset[_tempId].done(forcedCacheResponse.response.result,_tempDataset[_tempId].send);
											}
										} else {
											if (_tempDataset[_tempId].fail) _tempDataset[_tempId].fail(response,_tempDataset[_tempId].send);
										}
									};
							...
							if (_debug) {
								...
								// alert("FAIL 1: " + JSON.stringify(_data[this.id].send.params));
								// alert(JSON.stringify(response));
							}
							if (_data[this.id].fail) _data[this.id].fail(response,_data[this.id].send);
